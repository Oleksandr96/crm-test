...
export class AddEditUserComponent implements OnInit, OnDestroy {
  ...
  pinCode = new FormControl('', [Validators.required, Validators.minLength(4), Validators.pattern("^[0-9]*$")]);
  ...
  ngOnInit(): void {
    this.newUserSubscription = this.commonService.getUpdate().subscribe((user: User) => {
      this.userId = user._id || '';
      if (this.userId)
        this.isAddMode = false;
      this.usersService.getById(user._id)
        .pipe(first())
        .subscribe((user: User) => {
          console.log(user)
  ...
  private updateUser() {
    ...
    this.usersService.update(user, this.userId).subscribe((user: User) => {
      this.commonService.showNotify(`User ${user.firstName} ${user.lastName} updated`, 'Close')
  ...
  private createUser(): void {
    ...
    this.usersService.create(user).subscribe((user: User) => {
      this.commonService.showNotify(`User ${user.firstName} ${user.lastName} created`, 'Close')
      this.commonService.sendUpdate();
      this.formDirective.resetForm();
    });
  }

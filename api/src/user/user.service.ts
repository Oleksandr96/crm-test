...
export class UserService {
  ...
  async create(userDto: UserDto): Promise<User> {
    ...
    if (userByEmail) {
      throw new HttpException(`User with email ${userDto.emailID} already exist`,
  ...
  async findOneAndUpdate(userId: string, userDto: Partial<UserDto>): Promise<User> {
    const updatedUser = await this.userModel
      .findOneAndUpdate({ _id: userId }, userDto, { new: true });
    if (!updatedUser) {
      throw new HttpException('User does not exist', HttpStatus.NOT_FOUND)
    }
    return updatedUser;
